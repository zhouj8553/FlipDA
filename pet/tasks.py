    ...
    def _create_examples(self, path: str, set_type: str, hypothesis_name: str = "hypothesis",
        ...
        with open(path, encoding='utf8') as f:
            for line_idx, line in enumerate(f):
                ...
                orig_label = example_json.get('orig_label',label)
                guid = "%s-%s" % (set_type, idx)
                text_a = example_json[premise_name]
                text_b = example_json[hypothesis_name]

                example = InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, idx=idx, orig_label=orig_label)
                examples.append(example)
    ...
    def _create_examples(path: str, set_type: str) -> List[InputExample]:
        ...
        with open(path, encoding='utf8') as f:
            for line in f:
                ...
                if example_json.get('orig_label') is None:
                    orig_label=label
                else:
                    orig_label = "T" if example_json.get('orig_label') else "F"
                guid = "%s-%s" % (set_type, idx)
                text_a = example_json['sentence1']
                text_b = example_json['sentence2']
                meta = {'word': example_json['word']}
                example = InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, idx=idx, meta=meta, orig_label=orig_label)
                examples.append(example)
        return examples
    ...
    def _create_examples(path: str, set_type: str) -> List[InputExample]:
        ...
        with open(path, encoding='utf8') as f:
            for line in f:
                ...
                orig_label = str(example_json['orig_label']) if 'orig_label' in example_json else label
                guid = "%s-%s" % (set_type, idx)
                ...
                example = InputExample(guid=guid, text_a=text_a, label=label, meta=meta, idx=idx, orig_label=orig_label)
    ...
    def _create_examples(path: str, set_type: str) -> List[InputExample]:
        ...
        if set_type == 'train' or set_type == 'unlabeled':
            ...
            for ex in examples:
                ...
                orig_label = "1" if ex.orig_label == "0" else "0"
                meta = {
                    'choice1': ex.meta['choice2'],
                    'choice2': ex.meta['choice1'],
                    'question': ex.meta['question']
                }
                mirror_example = InputExample(guid=ex.guid + 'm', text_a=ex.text_a, label=label, meta=meta, orig_label=orig_label)
            ...
            for line in f:
                ...
                for question_json in questions:
                    ...
                    for answer_json in answers:
                        ...
                        orig_label =str(answer_json["orig_label"]) if "orig_label" in answer_json else label
                        ...
                        example = InputExample(guid=guid, text_a=text, text_b=question, label=label, meta=meta, idx=idx, orig_label=orig_label)
                        examples.append(example)
