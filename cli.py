...
def main():
    ...
    parser.add_argument("--search_type",default='',type=str) #jing
    parser.add_argument("--aug_ids", default=[0,2], type=int, nargs='+',) #jing
    parser.add_argument("--filter_pattern",default=-1,type=int) #jing
    parser.add_argument("--fixla_ratio",default='[[0.1,0.1],[0.1,0.1]]',type=str) #jing
    parser.add_argument("--fixla_num",default='[[14,14],[18,18]]',type=str) #jing
    parser.add_argument("--rmdup_num",default=1,type=int) #jing
    ...
    if args.use_noisy_student:
        ...
        unlabeled_data = processor._create_examples(
                os.path.join('/'.join(args.data_dir.split('/')[:-1]), "augmented/{}/{}".format(taskname_map[args.task_name], args.t5_augment_file_path)), "t5_augment_as_unlabeled")
        print("t5_augment_as_unlabeled num_examples: {}".format(len(unlabeled_data)))
    ...
    if args.search_type=='baseline' or args.search_type.startswith('mixup'):
        pass
    elif args.search_type.startswith('genaug'): #jing
        taskname_map={'copa':'COPA','rte':'RTE','boolq':"BoolQ",'multirc':"MultiRC",'wic':'WiC','wsc':"WSC",'cb':'CB'}
    ...
    elif args.search_type.startswith('myspecial'):
        augmented_data=pd.read_csv('results/human_augmented/aug_{}.csv'.format(args.task_name),encoding='gbk')
        data=[]
        for (i,e) in enumerate(train_data):
            for k in args.aug_ids:
                if args.task_name=='copa':
                    if i<32:
                        j=i*3+k
                        tmp_e=copy.deepcopy(e)
                        tmp_e.text_a=augmented_data['text_a'][j]
                        tmp_e.meta['choice1']=augmented_data['choice1'][j]
                        tmp_e.meta['choice2']=augmented_data['choice2'][j]
                    else:
                        j=(i-32)*3+k
                        tmp_e=copy.deepcopy(e)
                        tmp_e.text_a=augmented_data['text_a'][j]
                        tmp_e.meta['choice1']=augmented_data['choice2'][j]
                        tmp_e.meta['choice2']=augmented_data['choice1'][j]
                elif args.task_name=='wsc':
                    j=i*4+k
                    tmp_e=copy.deepcopy(e)
                    tmp_e.text_a=augmented_data['text_a'][j]
                    tmp_e.meta['span1_index']=augmented_data['span1_index'][j]
                    tmp_e.meta['span2_index']=augmented_data['span2_index'][j]
                    tmp_e.meta['span1_text']=augmented_data['span1_text'][j]
                    tmp_e.meta['span2_text']=augmented_data['span2_text'][j]
                elif args.task_name=='wic':
                    j=i*4+k
                    tmp_e=copy.deepcopy(e)
                    tmp_e.text_a=augmented_data['text_a'][j]
                    tmp_e.text_b=augmented_data['text_b'][j]
                    tmp_e.meta['word']=augmented_data['word'][j]
                    ...
                    tmp_e.text_a=augmented_data['text_a'][j]
                    tmp_e.text_b=augmented_data['text_b'][j]
                    tmp_e.label=augmented_data['label'][j]
                else:
                    j=i*5+k
                    tmp_e=copy.deepcopy(e)
                    tmp_e.text_a=augmented_data['text_a'][j]
                    ...
                    if args.task_name=='boolq':
                        tmp_e.label=str(augmented_data['label'][j])
                    else:
                        tmp_e.label=augmented_data['label'][j]
                data.append(tmp_e)
        train_data=data
    ...
    if 'filter' in args.search_type:
        pattern_iter_output_dir='new_results/baseline/pet/{}_{}_model/'.format(args.task_name.lower(), args.model_type)
        from genaug import confidence_filter
        if args.filter_pattern==-1:
            
            best_pattern_map={'boolq':4,'rte':10,'cb':3,'multirc':0,'wic':2,'wsc':2,'copa':0}
            best_iter_map={'boolq':0,'rte':1,'cb':0,'multirc':2,'wic':2,'wsc':0,'copa':0}
            
            args.filter_pattern=best_pattern_map[args.task_name.lower()]
            args.best_iter_pattern=best_iter_map[args.task_name.lower()]

        pattern_iter_output_dir=os.path.join(pattern_iter_output_dir,'p{}-i{}'.format(args.filter_pattern,args.best_iter_pattern))
        ...
        if 'flip' in args.search_type and 'max_otherla' not in args.search_type and args.task_name.lower()!='wsc':
            ...
            examples=new_examples
        # if args.task_name.lower()=='copa':
        #    new_examples,filtered_num=myfilter.recover_copa_labels(myfilter.wrapper,examples,pet_eval_cfg,recover_type=args.search_type.split('_filter_')[1],orig_data=train_data)
        # else:
    ...
    if args.method == 'pet':
        jing_results=pet.train_pet(pet_model_cfg, pet_train_cfg, pet_eval_cfg, sc_model_cfg, sc_train_cfg, sc_eval_cfg,
        ...
        if args.pet_repetitions!=1:
            if args.search_type.startswith('genaug'):
                writer=open(os.path.join('new_results/','pet_total_genaug_rep{}_{}.csv'.format(args.pet_repetitions,args.task_name)),'a+')
            else:
                writer=open(os.path.join('new_results/','pet_total_rep{}_{}.csv'.format(args.pet_repetitions,args.task_name)),'a+')
        else:
            if args.search_type.startswith('genaug'):
                writer=open(os.path.join('new_results/','pet_total_genaug_{}.csv'.format(args.task_name)),'a+')
            else:
                writer=open(os.path.join('new_results/','pet_total_{}.csv'.format(args.task_name)),'a+')
        writer.write((': {}, '.join(template_name)+': {}\n').format(*template_values+[jing_results]+[mean_result]))
        writer.close()
        ###################################### jing ###########################################
